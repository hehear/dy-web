package com.dy.u.file.rest;

import com.dy.s.basic.util.Md5Util;
...
public class FileRest {
    ...
    public String index(Model model) {
        // 展示最新二十条数据
        model.addAttribute("files", IFileService.listFilesByPage(0, 20));
        return "file";
    }
    ...
    public List<File> listFilesByPage(@PathVariable int pageIndex, @PathVariable int pageSize) {
        return IFileService.listFilesByPage(pageIndex, pageSize);
    }
    ...
    public ResponseEntity<Object> serveFile(@PathVariable String id) throws UnsupportedEncodingException {
        ...
        Optional<File> file = IFileService.getFileById(id);
    ...
    public ResponseEntity<String> handleFileUpload(@RequestParam("file") MultipartFile file) {
        ...
        try {
            File f = new File(file.getOriginalFilename(), file.getContentType(), file.getSize(),
                    new Binary(file.getBytes()));
            f.setMd5(Md5Util.getMD5(file.getInputStream()));
            returnFile = IFileService.saveFile(f);
